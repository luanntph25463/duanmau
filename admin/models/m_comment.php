...
class comment_class
{
    ...
    {
        $query = "select hh.id,hh.productName,count(*) quantity,min(create_at),max(create_at) from binhluan bl join products hh on hh.id = bl.id_product GROUP BY hh.id,hh.productName having quantity >0;";
        $result = $this->db->select($query);
        return $result;
    }
    public function show_cmmts($id)
    {
        $query = "select bl.*,kh.taikhoan,kh.hoten,kh.hinhanh from binhluan bl join khachhang kh on kh.id = bl.id_user where bl.id_product = '$id' order by bl.id desc";
        $result = $this->db->select($query);
        return $result;
    }
